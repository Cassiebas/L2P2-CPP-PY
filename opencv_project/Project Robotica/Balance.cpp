...
void getOffsets() {
  ...
  printf("Calculating offsets!\n");
	for (int i = 0; i < 200; i++) { //Use loop to average offsets
	...
	for(int o = 0; o <= 1; o++)
	{
	accel_off[0] = i2c_read(0x3B) << 8 | i2c_read(0x3C); //Read X registers
    accel_off[1] = i2c_read(0x3D) << 8 | i2c_read(0x3E); //Read Y registers
    accel_off[2] = i2c_read(0x3F) << 8 | i2c_read(0x40); //Read Z registers
    
    accel_off[0] = two_complement_to_int(accel_off[0]);
    accel_off[1] = two_complement_to_int(accel_off[1]);
    accel_off[2] = two_complement_to_int(accel_off[2]);
    
    ax_off = ax_off + accel_off[0];
    ay_off = ay_off + accel_off[1]; 
    az_off = az_off + accel_off[2];
	}
	gr_off = gr_off / 200, gp_off = gp_off / 200, gy_off = gy_off / 200; //Divide by number of loops (to average)
	ax_off = ax_off / 1, ay_off = ay_off / 1, az_off = az_off / 1;
	//printf("%d", ax_off);
...
int main(int argc, char *argv[]) {
  ...
  getOffsets();
    ...
    char flag = 1;
    float firstmeasurex = 0, firstmeasurey = 0;
    ...
    while(1)
    {
      ...
      */
      ...
      /*
      AX = (round(AX - ax_off)); //might want to change 1 to a higher number
      AY = (round(AY - ay_off));
      AZ = (round(AZ - az_off));
      */
